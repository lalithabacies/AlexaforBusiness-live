...
*/
    ...
    class PilotPress {
        ...
        function __construct() {
            ...
            {
                $this->stashed_transients["pilotpress_admin_preview"] = array(get_transient("pilotpress_admin_preview"));
        ...
        function load_settings() {
            ...
            if(get_transient('pilotpress_cache')) {
                ...
                $this->ppp->ppprotectSetPPMemLevels($this->get_setting("membership_levels", "oap"));
        ...
        function settings_init() {
            ...
            add_settings_field('pilotpress_newly_registered_campaigns', __('What campaigns should they be on?', 'pilotpress'), array(&$this, 'display_settings_newly_registered_campaigns'), 'pilotpress-settings', 'pilotpress-new-user-display');
            ...
            <script type="text/javascript">
                ...
                jQuery(function($)
                {
                    ...
                    purge_btn.click(function()
                    {
                        ...
                        var data = {'action':'purge_transients'};
                        if(conf == true)
                        {
        ...
        private function bind_hooks() {
            ...
            if(is_admin()) {
                ...
                add_action('edit_form_top', array(&$this, 'admin_preview'));
        ...
        function admin_preview()
        {
            ...
            //post isn't publicly accessible -- don't allow a preview
            else
            {
                echo "jQuery(function($)
                    {
        ...
        {
               set_transient("pilotpress_admin_preview", $_POST['value'], self::TTL);
                $data = array('data' => get_permalink($_POST["post"]));
        ...
        function get_stashed($name, $unique)
        {
            ...
            if(isset($this->stashed_transients[$name.$suffix]))
            {
                return $this->stashed_transients[$name.$suffix];
            }
            ...
            $this->stashed_transients[$name.$suffix] = $transient;
        ...
        function destroy_transients_logout()
        {
            ...
            delete_transient("login_url_pilotpress_user".$_COOKIE["contact_id"]);
            delete_transient("pilotpress_redirect_to".$_COOKIE["contact_id"]);
        ...
        */
            ...
            if(isset($_POST["wp-submit"])) {
                ...
                if(is_array($api_result)) {
                    ...
                    if($user->user_level != 10) {
                        ...
                        set_transient("login_url_pilotpress_user".$_COOKIE["contact_id"], $referrer, self::TTL);
                        ...
                        $redirect_to = get_transient("pilotpress_redirect_to".$_COOKIE["contact_id"]);
                        if(isset($redirect_to) && !empty($redirect_to) && !strpos($referrer, 'wp-login.php')) {
                            ...
                            delete_transient("pilotpress_redirect_to");
        ...
        function user_logout() {
            ...
            $redirect_to = get_transient("login_url_pilotpress_user".$_COOKIE["contact_id"]);
            $this->destroy_transients_logout();
        ...
        function throttle($options)
        {
            if (is_array($options) && isset($options['id']) && isset($options['throttled'])) 
            {
                ...
                if(!$admin)
                {
                    ...
                    $throttle_data = get_transient("pilotpress_throttle".$_COOKIE['contact_id']);
                    ...
                    set_transient("pilotpress_throttle".$_COOKIE['contact_id'], $throttle_data, self::TTL);
                }
            }
        }
        ...
        function DoShortcodeMagic($atts,$content)
        {
            ...
            if(current_user_can("manage_options") && isset($this->stashed_transients["pilotpress_admin_preview"]))
            {
                $user_levels = $this->stashed_transients["pilotpress_admin_preview"];
            }
            else
            {
            ...
            if(isset($atts["level"])) {
                ...
                if(isset($atts["has_tag"])) {
                    ...
                    if(is_array($tags)) {
                        ...
                        if(in_array(strtolower($atts["has_tag"]), $lower)) {
                            return '<span class="pilotpress_protected">'.do_shortcode($content).'</span>';
                        }
                    }
                }
        ...
        function post_process ()
        {
            ...
            if (!$this->is_viewable($id) && !is_home() 
                || get_user_meta(get_current_user_id(), "pilotpress_blocked", true) == "yes")
            {
                ...
                if (!empty($redirect))
                {
                    ...
                    else if ($redirect == "-2")
                    {
                        if (!empty($id))
                        {
                            ...
                            set_transient("pilotpress_redirect_to".$_COOKIE["contact_id"], $id, self::FIVE_MINUTES);
                        }
                    }
                    $_SESSION["redirect_to"] = $id;
                    set_transient("pilotpress_redirect_to".$_COOKIE["contact_id"], $id, self::FIVE_MINUTES);
        ...
        function posts_where($where) {
            ...
            else if(current_user_can('manage_options') && isset($this->stashed_transients["pilotpress_admin_preview"]))
            {
                $user_levels = $this->stashed_transients["pilotpress_admin_preview"];
            }
            else
        ...
        function do_system_page($id) {
            ...
            if($type == "affiliate_center") {
                $api_result = $this->api_call("get_".$type, array("username" => $user_info["authenticate_user"]["username"], "program_id" => $user_info["authenticate_user"]["program_id"], "site" => site_url()  , "styles"=>$styles ));
            }  
